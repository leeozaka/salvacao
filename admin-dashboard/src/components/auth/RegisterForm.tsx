...
import { isValidCPF, isValidPhone, isValidPassword, formatCPF, formatPhone } from '@/utils/validationUtils';
...
export default function RegisterForm({ onSuccess, onToggleForm }: RegisterFormProps) {
  ...
  const validateForm = (): boolean => {
    ...
    if (!isValidPassword(formData.password)) {
      newErrors.push('Password must have at least 8 characters with uppercase, lowercase, numbers and special characters');
    }
    ...
    if (!isValidPhone(formData.telephone)) newErrors.push('Invalid phone number format');
